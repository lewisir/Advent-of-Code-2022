...
def main():
    ...
    for name, valve in valve_dict.items():
        temp_path = ValvePath()
        if valve.rate != 0:
            new_path_list = discover_paths(
                (valve, valve.vectors["AA"]), TIME, valve_dict, temp_path
            )
            all_paths.extend(new_path_list)
        del temp_path
    for path in all_paths:
        flow = path.path_flow(TIME)
        if flow > max_flow:
            max_flow = flow
            print(f"new max flow {flow}")
            path.display_path(TIME)
            print()
    print(f"Part I - Max flow found {max_flow}")
